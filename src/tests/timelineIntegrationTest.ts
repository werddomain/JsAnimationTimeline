 ...
 */
function testHorizontalScroll(keyframesContainer: HTMLElement, timeRuler: HTMLElement): void {
    // Scroll positions to test
    const positions: number[] = [100, 200, 500, 1000];
    ...
    positions.forEach(pos => {
        ...
        const rulerContent = timeRuler.firstElementChild as HTMLElement;
        const transform = getComputedStyle(rulerContent).transform;
 ...
 */
function testVerticalScroll(keyframesContainer: HTMLElement, layersContainer: HTMLElement): void {
    // Scroll positions to test
    const positions: number[] = [50, 100, 150, 200];
 ...
 */
function testPlayheadAlignment(keyframesContainer: HTMLElement): void {
    // Get playhead element
    const playheadLine = document.querySelector('.timeline-playhead-line') as HTMLElement | null;
 ...
 */
function testLayerSelectionVisibility(layersContainer: HTMLElement, keyframesContainer: HTMLElement): void {
    ...
    const layerToSelect = layerRows.length > 1 ? layerRows[1] as HTMLElement : layerRows[0] as HTMLElement;
 ...
 */
function testTweenDisplay(keyframesContainer: HTMLElement): void {
    ...
    tweens.forEach((tween, index) => {
        const tweenElement = tween as HTMLElement;
        // Check that the tween has a width
        const width = tweenElement.offsetWidth;
        ...
        const computedStyle = getComputedStyle(tweenElement, ':after');
 ...
 */
function testPluginCommunication(): void {
 ...
 */
function testSelectionSynchronization(): void {
