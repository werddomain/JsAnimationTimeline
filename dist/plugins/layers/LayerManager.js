...
export class LayerManager extends Component {
    ...
    renderLayer(layer, indentLevel = 0) {
    ...
    handleClick(e) {
        ...
        if (target.closest('.timeline-layer-btn')) {
            ...
            const action = button === null || button === void 0 ? void 0 : button.getAttribute('data-action');
            switch (action) {
    ...
    handleMouseDown(e) {
        ...
        if (!target.closest('.timeline-layer-drag-handle'))
            return;
        const layerEl = this.findParentWithClass(target, CSS_CLASSES.LAYER);
        if (!layerEl)
            return;
    ...
    handleOutsideClick(e) {
        if (this.colorPickerElement && !this.colorPickerElement.contains(e.target)) {
     ...
     */
    promptLayerNameEdit(layerEl, layerId) {
     ...
     */
    promptColorChange(layerId, targetElement) {
