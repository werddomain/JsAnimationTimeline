...
export class PanelComponent extends Component {
    ...
    initialize() {
        ...
        this.eventEmitter.on(EVENTS.TIME_CHANGE, (time) => {
 ...
 */
    ...
    handleOutsideClick(e) {
        var _a;
        const panelContent = (_a = this.getElement()) === null || _a === void 0 ? void 0 : _a.querySelector('.timeline-panel-content');
    ...
    updateElementProperty(elementId, property, value) {
        ...
        if (!element.modifiedProperties) {
            element.modifiedProperties = new Set();
        }
    ...
    applyElementProperties(element) {
        ...
        const transforms = [];
        if (element.properties.translateX)
            transforms.push(`translateX(${element.properties.translateX}px)`);
        if (element.properties.translateY)
            transforms.push(`translateY(${element.properties.translateY}px)`);
        if (element.properties.scaleX)
            transforms.push(`scaleX(${element.properties.scaleX})`);
        if (element.properties.scaleY)
            transforms.push(`scaleY(${element.properties.scaleY})`);
        if (element.properties.skewX)
            transforms.push(`skewX(${element.properties.skewX}deg)`);
        if (element.properties.skewY)
            transforms.push(`skewY(${element.properties.skewY}deg)`);
        if (transforms.length > 0) {
            styles += `transform: ${transforms.join(' ')}; `;
        }
        // Add filter property if any filter values exist
        const filters = [];
        if (element.properties.blur)
            filters.push(`blur(${element.properties.blur}px)`);
        if (element.properties.brightness)
            filters.push(`brightness(${element.properties.brightness}%)`);
        if (element.properties.contrast)
            filters.push(`contrast(${element.properties.contrast}%)`);
        if (element.properties.grayscale)
            filters.push(`grayscale(${element.properties.grayscale}%)`);
        if (element.properties.hueRotate)
            filters.push(`hue-rotate(${element.properties.hueRotate}deg)`);
        if (element.properties.invert)
            filters.push(`invert(${element.properties.invert}%)`);
        if (element.properties.saturate)
            filters.push(`saturate(${element.properties.saturate}%)`);
        if (element.properties.sepia)
            filters.push(`sepia(${element.properties.sepia}%)`);
        if (filters.length > 0) {
     ...
     */
    getPropertyUnit(property, element) {
    ...
    updateElementKeyframe(element) {
        ...
        element.modifiedProperties.forEach(prop => {
            if (['x', 'y', 'width', 'height', 'rotation', 'opacity', 'zIndex'].includes(prop)) {
                keyframeProperties[prop] = element[prop];
            }
